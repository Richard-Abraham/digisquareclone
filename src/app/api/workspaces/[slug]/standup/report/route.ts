...
export async function POST(req: NextRequest, { params }: { params: { slug: string } }) {
  try {
    ...
    if (!checkRateLimit(`standup-report:post:${getClientKey(req)}`, { windowMs: 60_000, maxRequests: 30 })) {
    ...
    if (items.length) {
      ...
      if (completedIds.length) {
        ...
        await Promise.all(completedIds.map((issueId) => {
          ...
          return writeActivity({ workspaceId: wsId, actorId: user.id, kind: "completed", targetType: "issue", issueId, projectId }).catch((e) => { logger.warn("writeActivity failed", undefined, e); });
        }));
      }
    }
    return ok({ ok: true, submitted: !!submit });
  } catch (e) {
    logger.error("POST /api/workspaces/[slug]/standup/report failed", e);
