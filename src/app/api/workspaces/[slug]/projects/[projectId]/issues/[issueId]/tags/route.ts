...
export async function PUT(req: NextRequest, { params }: { params: { projectId: string; issueId: string } }) {
  try {
    ...
    if (!checkRateLimit(`issue-tags:put:${getClientKey(req)}`, { windowMs: 60_000, maxRequests: 30 })) {
    ...
    logger.error("PUT /api/workspaces/[slug]/projects/[projectId]/issues/[issueId]/tags failed", e);
