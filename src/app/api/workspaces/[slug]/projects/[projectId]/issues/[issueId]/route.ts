...
export async function GET(req: NextRequest, { params }: { params: { slug: string; projectId: string; issueId: string } }) {
  try {
    ...
    if (!checkRateLimit(`issue:get:${getClientKey(req)}`, { windowMs: 60_000, maxRequests: 30 })) {
    ...
    logger.error("GET /api/workspaces/[slug]/projects/[projectId]/issues/[issueId] failed", e);
...
export async function PATCH(req: NextRequest, { params }: { params: { slug: string; projectId: string; issueId: string } }) {
  try {
    const user = await getUser(req);
    if (!user) return err("Unauthorized", 401);
    if (!checkRateLimit(`issue:patch:${getClientKey(req)}`, { windowMs: 60_000, maxRequests: 30 })) {
      return err("Too many requests", { status: 429 });
    }
    if (!(await getProjectAccess(params.projectId, user.id))) return err("Access denied", 403);
    ...
    logger.error("PATCH /api/workspaces/[slug]/projects/[projectId]/issues/[issueId] failed", e);
    return err("Internal server error", { status: 500 });
  }
}
...
export async function DELETE(req: NextRequest, { params }: { params: { slug: string; projectId: string; issueId: string } }) {
  try {
    const user = await getUser(req);
    if (!user) return err("Unauthorized", 401);
    if (!checkRateLimit(`issue:delete:${getClientKey(req)}`, { windowMs: 60_000, maxRequests: 30 })) {
      return err("Too many requests", { status: 429 });
    }
    if (!(await getProjectAccess(params.projectId, user.id))) return err("Access denied", 403);
    ...
    logger.error("DELETE /api/workspaces/[slug]/projects/[projectId]/issues/[issueId] failed", e);
    return err("Internal server error", { status: 500 });
  }
}
