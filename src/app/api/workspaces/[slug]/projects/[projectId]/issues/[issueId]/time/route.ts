...
export async function GET(req: NextRequest, { params }: { params: { projectId: string; issueId: string } }) {
  try {
    ...
    if (!checkRateLimit(`issue-time:get:${getClientKey(req)}`, { windowMs: 60_000, maxRequests: 30 })) {
    ...
    logger.error("GET /api/workspaces/[slug]/projects/[projectId]/issues/[issueId]/time failed", e);
...
export async function POST(req: NextRequest, { params }: { params: { projectId: string; issueId: string } }) {
  try {
    const user = await getUser(req);
    if (!user) return err("Unauthorized", 401);
    if (!checkRateLimit(`issue-time:post:${getClientKey(req)}`, { windowMs: 60_000, maxRequests: 30 })) {
      return err("Too many requests", { status: 429 });
    }
    if (!(await getProjectAccess(params.projectId, user.id))) return err("Access denied", 403);
    ...
    logger.error("POST /api/workspaces/[slug]/projects/[projectId]/issues/[issueId]/time failed", e);
    return err("Internal server error", { status: 500 });
  }
}
