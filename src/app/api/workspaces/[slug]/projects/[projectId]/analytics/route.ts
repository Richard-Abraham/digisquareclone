...
export async function GET(req: NextRequest, { params }: { params: { slug: string; projectId: string } }) {
  try {
    ...
    if (!checkRateLimit(`project-analytics:get:${getClientKey(req)}`, { windowMs: 60_000, maxRequests: 30 })) {
    ...
    logger.error("GET /api/workspaces/[slug]/projects/[projectId]/analytics failed", e);
